...
enum GameCatalog {
    ...
    static func shelves(favorites: [TrackedGame], recents: [TrackedGame]) -> [(String, [CatalogGame])] {
        ...
        if !recents.isEmpty {
            let mapped = recents.compactMap { recent in
                games.first(where: { $0.id == recent.id }) ?? CatalogGame(
                    id: recent.id, slug: recent.slug, title: recent.title,
                    tagline: "Recently played", genre: "Cloud", provider: "Xbox Cloud",
                    featured: false, accent: 0x4361EE
                )
            }
            rows.append(("Continue playing", mapped))
        }
        if !favorites.isEmpty {
            let mapped = favorites.compactMap { fav in
                games.first(where: { $0.id == fav.id }) ?? CatalogGame(
                    id: fav.id, slug: fav.slug, title: fav.title,
                    tagline: "Pinned to your hub", genre: "Favorites", provider: "Xbox Cloud",
                    featured: false, accent: 0xF4A261
                )
            }
            rows.append(("Favorites", mapped))
        }
