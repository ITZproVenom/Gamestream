...
struct GameHubView: View {
    ...
    /// Hero first, continue strip, activity, then rails.
    private var editorialHome: some View {
        VStack(alignment: .leading, spacing: appearance.density.sectionSpacing) {
            if let hero = GameCatalog.featured.first {
                ...
                .frame(maxWidth: .infinity)
                .frame(height: appearance.density == .compact ? 180 : 240)
            }
    ...
    private func handleGridNav(_ action: ControllerGameNav) {
        ...
        case .right: next = min(current + 1, games.count - 1)
        case .up: next = max(current - 2, 0)
        case .down: next = min(current + 2, games.count - 1)
        default: return
        }
